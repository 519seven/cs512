...
class orderedArrayListType: public arrayListType<Type> {
 public:
    ...
    void insertEnd(const Type& insertItem, const bool f);
    void replaceAt(int location, const Type& repItem);
    void remove(const Type& removeItem);

    // Searching
    int seqSearch(const Type& searchItem);
    int recursiveBinarySearch(int f, int l, const Type& x);
    int doBinary(const Type& searchItem);

    // Sorting
    int minLocation(int f, int e);
    void swapIt(int f, int s);
    void selectionSort();
    void insertionSort();
...
void orderedArrayListType<Type>::populateRandomly() {
    ...
    while (this->length < this->maxSize) {
        r = rand()%1000;          // cpplint says use rand_r but is it portable?
        insertEnd(r, force);
    }
...
// begin insertAt
template <class Type>
void orderedArrayListType<Type>::insertAt
...
// begin insertEnd
template <class Type>
void orderedArrayListType<Type>::insertEnd
        (const Type& insertItem, const bool force) {
...
// begin replaceAt
template <class Type>
void orderedArrayListType<Type>::replaceAt(int location, const Type& repItem) {
...
// begin seqSearch
template <class Type>
int orderedArrayListType<Type>::seqSearch(const Type& searchItem) {
    int loc;
    bool found = false;
...
template <class Type>
int orderedArrayListType<Type>::recursiveBinarySearch
        (int f, int l, const Type& x) {
    this->keyComparisons++;
    if (l-f >= 1) {
        int mid = (f+l)/2;
...
// begin doBinary
template <class Type>
int orderedArrayListType<Type>::doBinary(const Type& item) {
...
template <class Type>
void orderedArrayListType<Type>::swapIt(int first, int second) {
  Type temp;
